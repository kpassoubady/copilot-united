...
Why it's a good exercise: Teaches API consumption, JSON parsing, in-memory caching (TTL), rate limiting awareness, error handling patterns, and lightweight service design without being too advanced.
...
Why it's a good exercise: Teaches graphs, priority queues, heuristics, and makes abstract algorithms tangible through visualization.
...
Why it's a good exercise: Practices text parsing, regular expressions, state handling, and file I/O.

## 6. Concurrent File Downloader ⚡

Given a list of file URLs (e.g., images), download them concurrently using threads or async tasks. Show elapsed time vs sequential.

Why it's a good exercise: Introduces concurrency, I/O-bound optimization, thread/async management, and performance measurement.
...
Why it's a good exercise: Reinforces hash maps/dictionaries, serialization, simple protocol design, and REPL-style input handling.

## 8. Client-Server Chat Application 💬

Create a multi-client chat system. Server broadcasts each received message to all connected clients.

Why it's a good exercise: Covers sockets, concurrency (threads or async), message framing, and basic protocol design.
...
Why it's a good exercise: Practices tokenization, set operations, file I/O, and applying a mathematical measure to text analysis.
...
Why it's a good exercise: Combines API design, persistence, string manipulation, and HTTP redirection semantics.
